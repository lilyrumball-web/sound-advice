...
const TRIALS_PER_BLOCK = 125;          // 125 x 1.2 s = 2 min 30
...
async function boot() {
  ...
  buildScenes();                      // draw the test images while the phone is idle
...
function renderSettings() {
  ...
  Promise.all(music.map(audio.fileAvailable)).then(av => {
    ...
    $('creditMusic').textContent = added.length
      ? 'Tracks are listed in the audio/CREDITS.txt file that ships with the app.'
